...
const Organizations = () => {
  ...
  const [aOrgs,setAOrgs] = useState([]);
  const [uOrgs,setUOrgs] = useState([]);
  ...
  useEffect(()=>{
    const fetchApplicants = async() =>{
     ...
     const approvedOrgs = res.data.filter(org => org.approvedByAdmin === true);
     const unapprovedOrgs = res.data.filter(org => org.approvedByAdmin === false);
    ...
    const apporval = await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/org/updateOrganization/${org._id}`,data,{
        headers: {
          'Authorization': localStorage.getItem("token"),
        },
        withCredentials: true
